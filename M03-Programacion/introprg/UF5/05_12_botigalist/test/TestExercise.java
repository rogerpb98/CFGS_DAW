...
public class TestExercise {
    ...
    class SobreBotiga {
        ...
        public void cercaTrobaViExistentDesDePlantilla(String ref,
            ...
            List<Vi> esperat = Arrays.asList(vins[indexEsperat]);
            Vi plantilla = new Vi(ref, nom, tipus, preu, collita, estoc, origen, lloc);
            List<Vi> trobat = botiga.cerca(plantilla);
            assertAll(
                    () -> assertNotNull(trobat, "Botiga.cerca() no ha de tornar mai null!"),
                    () -> assertFalse(trobat.isEmpty(), "Botiga.cerca() no està trobant un vi que hi és"),
                    () -> assertEquals(1, trobat.size(), "Botiga.cerca() està trobant vins no demanats"),
                    () -> assertEquals(esperat.get(0).getRef(), trobat.get(0).getRef(), missatge),
                    () -> assertEquals(esperat.get(0).getNom(), trobat.get(0).getNom(), missatge),
                    () -> assertEquals(esperat.get(0).getTipus(), trobat.get(0).getTipus(), missatge),
                    () -> assertEquals(esperat.get(0).getPreu(), trobat.get(0).getPreu(), missatge),
                    () -> assertEquals(esperat.get(0).getCollita(), trobat.get(0).getCollita(), missatge),
                    () -> assertEquals(esperat.get(0).getEstoc(), trobat.get(0).getEstoc(), missatge),
                    () -> assertEquals(esperat.get(0).getOrigen(), trobat.get(0).getOrigen(), missatge),
                    () -> assertEquals(esperat.get(0).getLloc(), trobat.get(0).getLloc(), missatge)
                    );
        }

        @Test
        @DisplayName("test Vi.cerca() trobant vins amb un preu màxim")
        public void cercaVinsPreuMaxim() throws BotigaPlenaException {
            Vi[] vins = {
                new Vi("LLUMALBA20200001", "Llum Alba Blanc", "blanc", 1750, "2020", 12, "Priorat", "P02E03N55D"),
                new Vi("CARPATHI20180021", "Carpathia Negre", "negre", 3450, "2018", 6, "Montsant", "P23E01N43D"),
                new Vi("MATISNEG20190011", "Matís Negre", "negre", 1325, "2019", 12, "Pla del Bages", "P20E01N12E"),
            ...
            List<Vi> esperat = Arrays.asList(vins[0], vins[2], vins[3]);
            Vi plantilla = new Vi("", "", "", 3000, "", -1, "", "");
            List<Vi> trobat = botiga.cerca(plantilla);
            assertEquals(esperat, trobat, "revisa la cerca per preu màxim");
        }

        @Test
        @DisplayName("test Vi.cerca() trobant vins amb múltiples condicions")
        public void cercaVinsMultiplesCondicions() throws BotigaPlenaException {
            Vi[] vins = {
                new Vi("LLUMALBA20200001", "Llum Alba Blanc", "blanc", 1750, "2020", 12, "Priorat", "P02E03N55D"),
                new Vi("CARPATHI20180021", "Carpathia Negre", "negre", 3450, "2018", 6, "Montsant", "P23E01N43D"),
                new Vi("MATISNEG20190011", "Matís Negre", "negre", 1325, "2019", 12, "Pla del Bages", "P20E01N12E"),
            ...
            List<Vi> trobat = botiga.cerca(plantilla);
            assertEquals(esperat, trobat, "revisa la cerca quan la plantilla presenta múltiples condicions");
        }
